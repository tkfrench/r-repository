...
# Generate 50,000 random encounters 
# with [5 regions],[24 ministires], [2 startifiers] & [4 outcomes]

stratumVars         <- c("gender",   "age_gt65",'year', 'monseq')
...
mydata<-data.frame( 
  ...
  monseq   = sample(c(1:24),CaseCount,replace=T),     # Add month Sequence 
...
mydata$year     = 2016+as.integer(1+mydata$monseq/12) # Add year 

#Entity type summary results (mean, sd, n) by stratifiers by MoSeq for numeric outcomes 1 thru N
